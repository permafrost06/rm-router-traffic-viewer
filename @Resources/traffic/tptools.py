...
class Instance(object):	
	...
	def macresolver(self):
		...
		for item in ret:
			...
			resolve[self.escape(new[1])] = self.escape(new[0])
		return resolve
	...
	def ipresolver(self):
		...
		for item in ret:
			new = item.group(0).split(",\n")
			resolve[self.escape(new[2])] = self.escape(new[0])
		return resolve
		
	def stat(self):
		stat = []
		r = get("http://192.168.0.1/userRpm/SystemStatisticRpm.htm?sortType=5&Num_per_page=100", headers=self.auth).text.split("var statList = new Array(\n")[1].split("0,0 );")[0].split("\n")
		for line in r:
			if line != "":
				line = line.split(",")
				stat.append({
				'ip': self.escape(line[1]),
				'mac': self.escape(line[2]),
				'pact_total': line[3],
				'bytes_total': line[4],
				'pact_curr': line[5],
				'bytes_curr': line[6]
